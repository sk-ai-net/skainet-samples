...
fun DrawingScreen(handleSource: () -> Source) {
    ...
    var isChooseImage by remember { mutableStateOf(false) } // State for the switch

    var selectedImageIndex by remember { mutableStateOf(-1) } // State to remember the selected image index
    ...
    Column(
        ...
        // Show Image Grid if Switch is On
        if (isChooseImage) {
            LazyVerticalGrid(
                columns = GridCells.Fixed(5), // 5 columns in the grid
                modifier = Modifier
                    .fillMaxWidth()
                    .weight(1f), // Allow the grid to take available space
                content = {
                    items(imageResources.size) { index ->
                        Image(
                            ...
                            modifier = Modifier
                                .padding(4.dp)
                                .size(48.dp) // Set image size to 48dp x 48dp
                                .clickable {
                                    selectedImageIndex = index // Update selected image index
                                }
                                .border(
                ...
                Row {
                    ...
                    Button(
                        ...
                        colors = ButtonDefaults.buttonColors(
                            backgroundColor = if (isChooseImage) Color.Green else Color.Cyan // Change colors based on state
                        )
                    ) {
                        if (isChooseImage) {
                            Text("Draw image")
